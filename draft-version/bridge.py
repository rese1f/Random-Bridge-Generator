...
class RailCurve:
    def __init__(self, deck_width, deck_length, deck_height, deck_thickness, a, b):
        ...
        self.railslab_width = self.deck_width / 2 - self.deck_width / 4       
        ...
        self.sleeper_span = self.deck_length / 250        
...
class RailStraight:
    def __init__(self, deck_width, deck_length, deck_height, deck_thickness):
        self.deck_width = deck_width
        self.deck_length = deck_length
        self.deck_height = deck_height
        self.deck_thickness = deck_thickness
        ...
        self.sleeper_length = 0.2
        # self.sleeper_span = 0.5  
        self.sleeper_span = self.deck_length / 50
    ...
    def rail(self):
        ...
        for j in range(2):
            ...
            member.railSlabBuild()
            member.rail_slab.location.y = dis
        ...
        for i in range(self.sleeper_number):
            # ut.Duplicate(member.rail_sleeper, 'rail sleeper'+str(i+1), self.fx(int(i*self.sleeper_span),self.a,self.b) - dis*np.sin(theta),\
            #     self.fy(int(i*self.sleeper_span),self.a,self.b) + dis*np.cos(theta), None, 0, 0, np.arctan(theta))
            start = - self.deck_length / 2
            for j in range(2):
                dis = (-1) ** j * (self.deck_width*0.3 - self.railslab_width/2)            
                ut.Duplicate(member.rail_sleeper, 'rail sleeper'+str(i+1), start + i * self.sleeper_span, dis)
            ...
            member = cp.Railway(self.deck_width, self.deck_length, self.deck_height, self.deck_thickness,\
                 self.railslab_width, self.railslab_thickness, self.rail_width, self.rail_thickness,\
                    self.sleeper_height, self.sleeper_length,\
                        self.sleeper_span, 'rail track', t, quat) 
            member.railBuild()
            member.rail_track.location.y = dis
...
class BeamBridge:
    def __init__(self, l_deck = 200, h_deck = 11, t_deck = 1, w_deck = 10, \
        ...
        self.parapet()
        # self.rail()
        RailCurve(self.deck_width, self.deck_length, self.deck_height, self.deck_thickness, self.a, self.b)
#############################################################################################

    # TODO: why the parameters haven't been used
    def fx(self,x,a,b):
        return x
...
###############################################################################################
    ...
    def girder_longitude(self):
        # (-1)**i*self.deck_width/5*int((i+1)/2)
        for j in range(5):
...
class DeckArchBridge:
    ...
    def __init__(self,w_deck = 10, h_deck = 30, t_deck = 3, l_deck = 102, l_column = 2, w_column = 2):
        ...
        RailCurve(self.deck_width, self.deck_length, self.deck_height, self.deck_thickness, 0, 1)    
...
class CableStayedBridge:
    ...
    ########################################
        ...
        RailStraight(self.deck_width, self.deck_length, self.deck_height, self.deck_thickness)        
...
class SuspensionBridgeGenerator:
    def __init__(self, l_deck = 0, h_deck = 8, t_deck = 1, w_deck = 10, d_column = 0,\
        ...
        a = random.uniform(3, 6)
        self.dist_column = a * self.column_height
        self.deck_length = 2 * self.dist_column
    ...
    def fz1(self,x):
        return (0.9*self.column_height-self.deck_height)/(self.dist_column/2)**2*x**2+self.deck_height

    def fz2(self,x):
        return (0.9*self.column_height-1.2*self.deck_height)/(self.dist_column/2)**2*(x-self.dist_column/2)**2+1.2*self.deck_height

    def fz3(self,x):
        return (0.9*self.column_height-self.deck_height)/(self.dist_column/2)**2*(x-self.dist_column/2)**2+self.deck_height


    def column(self):

        for i in range(self.num_column):        
            member = cp.Column(self.column_width, self.column_height, self.column_thickness, \
                self.column_length, self.deck_height, self.deck_thickness, 'column')
    ...
    def deck(self):
        member = cp.Slab(self.deck_thickness, self.deck_height, self.deck_length, self.column_width+self.column_thickness, \
            self.column_thickness, 'deck', None,None,self.deck_width,None)
        ...
        for i in range(1,4):
            for j in range(1,3):
                if i == 1:
                    functionx = self.fx1
                    function = self.fz1
                    length = int(self.dist_column/2) + 1
                elif i == 2:
                    functionx = self.fx2
                    function = self.fz2
                    length = int(self.dist_column) + 1
                elif i == 3:
                    functionx = self.fx3
                    function = self.fz3
                    length = int(self.dist_column/2) + 1
                ...
                t[:,1] = (self.column_width/2 - self.column_thickness/2) * (-1)**j
                ...
                for m,n in enumerate(cable_loc):
                    member = ut.Duplicate(cables.cable, "cable_straight"+'m'+str(m)+'j'+str(j)+'i'+str(i), functionx(n), ((self.column_width/2 - self.column_thickness/2) * (-1)**j),
                    self.deck_height-0.1*self.deck_thickness, 0, -np.pi/2, 0, function(n) - self.deck_height, None, None)
        bpy.data.objects['cable_straight'].select_set(True)
        bpy.ops.object.delete()
