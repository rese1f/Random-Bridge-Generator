...
if __name__ == "__main__":
    ...
    CreateEnv_Depth(dmin=0.5,dmax=30.0,dpow=1.0)
    ...
    savepath = os.path.join(path,'RenderedImage')
    if not os.path.exists(savepath):
        os.makedirs(savepath)
    for j in range(XYZ.shape[0]):
        UpdateCameraSet(camset,loc=XYZ[j,:],rot=CamRot[i,:],rx=rx,ry=ry,f=f,clip_end=500)
