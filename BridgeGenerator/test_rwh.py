...
class Cable2d(SuperStructure):
    def __init__(self,n,name,h_deck,w_column,t_column,h_column, dist_column,\
         Part, mirror = True, cable_radius = 0.05):
        super().__init__()
        #self.n = n
        self.H_deck = h_deck 
        self.cable_radius = cable_radius
        ...
        t[:,1] = np.array([self.fy(i) for i in range(length)])
        t[:,2] = np.array([function(i) for i in range(length)])
        member = Circle2d(cfg, length, t, None,name)
    ...
    def fy(self, x):
        if self.mirror == 1:
            return self.W_column/2 - self.T_column/2
        elif self.mirror == 2:
            return -self.W_column/2 + self.T_column/2
...
class Cable2d_s(SuperStructure):
    def __init__(self,Cable2d,n,name,h_deck,dist_column,w_column,t_column,Part,\
        mirror, cable_span,cable_radius = 0.05):
        super().__init__()
        Cable2d.fx2
        ...
        t = np.zeros((self.n,3))
        t[:,0] = np.arange(self.n)
        member = Circle2d(cfg, self.n, t, None,name)
        member.t = t
        member.createObj(name)

        if self.Part == 1:
            functionx = Cable2d.fx1
            function =Cable2d.fz1
            length = int(self.dist_column/2)
        elif self.Part == 2:
            functionx = Cable2d.fx2
            function = Cable2d.fz2
            length = int(self.dist_column)
        elif self.Part == 3:
            functionx = Cable2d.fx3
            function = Cable2d.fz3
            length = int(self.dist_column/2)

        for j,i in enumerate(self.duplicateDistance(cable_span,length)):
            cable = bpy.data.objects[name]
            namezip = str(name) + str(j)
            cablezip = bpy.data.objects.new(namezip,cable.data)
            cablezip.location.x = functionx(i)
            cablezip.location.y = self.fy(i)
            cablezip.location.z = self.H_deck
            cablezip.scale[0] = function(i) - self.H_deck
...
class SuspensionBridgeGenerator:
    def __init__(self, h_column = 36, t_column = 1, l_column = 2,\
        w_column = 10, h_deck = 8, t_deck = 1, index_column = 1, index_deck = 1,\
            index_cable = 1, face_cable = 2, num_cable = 7, truss = 0):
    ...
    def mainCable(self):
        for i in range(1,4):
            for j in range(1,3):
                b = Cable2d(int(self.dist_column),'maincable',self.H_deck,\
                    self.W_column,self.T_column,self.H_column,self.dist_column,\
                        Part = i,mirror = j,cable_radius = 0.1)
                b
                Cable2d_s(b,2,'maincable1234',self.H_deck,self.dist_column,\
                    self.W_column,self.T_column,Part = i,mirror = j,\
                        cable_span = 2,cable_radius = 0.1)

    def cable(self):
        cable_start = np.array([0,0,0])
        cable_end = np.array([1,0,0])
        Cable(cable_start, cable_end, "cable", cable_radius = 0.05, num = 12)
...
bridge = SuspensionBridgeGenerator( h_column, t_column, l_column,\
        w_column, h_deck, t_deck, index_column, index_deck)
